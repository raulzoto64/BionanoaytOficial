...
export default function AdminChats() {
  ...
  useEffect(() => {
    loadChats();
    // Poll for global updates every 6 seconds para no chocar con el timeout del typing
  ...
  useEffect(() => {
    if (selectedChatId) {
      ...
      pollInterval.current = setInterval(loadMessages, 6000);
  ...
  const loadChats = async () => {
    try {
      ...
      // ✅ Eliminar chats duplicados por ID (soluciona definitivamente el error de keys repetidas)
      setChats(prev => {
        // Eliminar duplicados
        const uniqueChats = Array.from(
          new Map(data.map((chat: Chat) => [chat.id, chat])).values()
        ) as Chat[];
        
        // ✅ Solo actualizar si realmente hay cambios
        const hasChanges = JSON.stringify(prev) !== JSON.stringify(uniqueChats);
        return hasChanges ? uniqueChats : prev;
      });
    } catch (e) {
  ...
  const loadMessages = async () => {
    ...
    try {
      ...
      if (data && data.messages) {
        ...
        // ✅ SI EL CHAT ESTA ABIERTO MARCAR AUTOMATICAMENTE COMO LEIDO SI HAY NUEVOS MENSAJES
        const currentChat = chats.find(c => c.id === selectedChatId);
        if (currentChat && (currentChat.unread_count_agent || 0) > 0) {
          markAsRead(selectedChatId);
        }
        ...
        setChats(prev => prev.map(c => 
          c.id === data.chat.id ? ({ 
            ...c, 
            // ✅ CONVERTIR A BOOLEANO (MySQL devuelve INT 0/1)
  ...
  const markAsRead = async (id: string) => {
    try {
      await supabaseAPI.markChatAsRead(id, 'agent');
      
      // ✅ FUENTE DE VERDAD: Recargar lista de chats desde backend
      await loadChats();
    } catch (e) {
      console.error(`❌ [MARK AS READ] Error marcando chat ${id} como leido`, e);
    }
  };
  ...
  const filteredChats = chats.filter(chat => {
    ...
    const matchesFilter = filter === 'all' || chat.unread_count_agent > 0;
  ...
  return (
    <div className="flex h-screen w-full bg-white overflow-hidden absolute inset-0 m-0 p-0">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {chat.unread_count_agent > 0 && (
                      <span className="bg-orange-500 text-white text-[10px] px-1.5 py-0.5 rounded-full animate-pulse font-black min-w-[20px] text-center">
                        {chat.unread_count_agent}
                      </span>
                    )}
                  </div>
      ...
      <div className="flex-1 flex flex-col bg-white">
        {selectedChatId ? (
          <>
            ...
            <div 
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50/30 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
            >
            {/* ✅ FIX: Mensaje fantasma '0' - Solo renderizar si es array valido */}
            {Array.isArray(messages) && messages.map((msg) => {
                const isAdmin = msg.sender_type === 'admin' || msg.sender_type === 'agent';
                // ✅ LOGICA CORRECTA: basado directamente en el estado is_read del mensaje
                // Convertimos a boolean de forma segura (funciona tanto para number 0/1 como boolean nativo)
                const isSeen = isAdmin && !!msg.is_read;
              ...
              {/* ✅ FIX: Mensaje fantasma '0' - Convertir SIEMPRE a boolean para que React no renderice numeros */}
