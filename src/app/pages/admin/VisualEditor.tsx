...
export function AdminVisualEditor() {
  ...
  const activeSectionES = activeSectionESById || 
                        (activeSectionIndexByEN >= 0 && activeSectionIndexByEN < sectionsES.length ? sectionsES[activeSectionIndexByEN] : null) ||
                        (activeSectionENById ? { ...activeSectionENById, content: {} } : null);

  const activeSectionEN = activeSectionENById || 
                        (activeSectionIndexByES >= 0 && activeSectionIndexByES < sectionsEN.length ? sectionsEN[activeSectionIndexByES] : null) ||
                        (activeSectionESById ? { ...activeSectionESById, content: {} } : null);
            ...
            <div className="relative group w-full flex justify-center py-10 min-h-screen items-start">
              ...
              <div className={`relative flex items-center justify-center ${deviceView !== 'desktop' ? 'mx-auto' : 'w-full'}`}>
                ...
                <div
                  ...
                  className={`bg-white ease-out shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative flex-shrink-0 origin-top ${isResizing ? '' : 'transition-all duration-700'} ${deviceView === 'mobile' ?
